...
public class TarifaService {
  ...
  /**
   * Crea una tarifa nueva para un tipo de habitacion, cerrando automaticamente la que estaba
   * vigente para no dejar solapamientos: - Continua (sin fechaFin): arranca en fechaInicio (o hoy
   * si no se especifica) y no tiene fin. - Promocional (con fechaFin): ademas crea la tarifa de
   * continuacion que retoma el precio anterior a partir del dia siguiente a que termina la promo.
   */
  @Transactional
  public Tarifa crear(Tarifa tarifaRequest) {
    if (tarifaRequest.getTipoHabitacion() == null
        || tarifaRequest.getTipoHabitacion().getId() == null) {
    ...
    LocalDate fechaInicio =
        tarifaRequest.getFechaInicio() != null ? tarifaRequest.getFechaInicio() : LocalDate.now();
    ...
    if (esPromocional && !fechaFin.isAfter(fechaInicio)) {
      throw new IllegalArgumentException(
          "La fecha de fin de una tarifa promocional debe ser posterior a la fecha de inicio");
    }
    ...
    // (fechaFin antes que fechaInicio). En ese caso la anterior queda completamente
    // reemplazada.
    Optional<Tarifa> vigenteAnterior = obtenerVigente(tipoHabitacionId, fechaInicio);
    vigenteAnterior.ifPresent(
        vigente -> {
    ...
    if (esPromocional) {
      Double precioContinuacion =
          vigenteAnterior.map(Tarifa::getPrecioNoche).orElse(tarifaRequest.getPrecioNoche());
      Tarifa continuacion =
          Tarifa.builder()
  ...
  /**
   * Solo se puede borrar si queda al menos otra tarifa para ese tipo de habitacion (no puede quedar
   * un tipo sin tarifa). Si la que se borra es la vigente, la tarifa anterior (la de fechaInicio
   * mas reciente entre las que ya terminaron) vuelve a quedar vigente (fechaFin = null).
   */
  @Transactional
  public void eliminar(Integer id) {
    ...
    List<Tarifa> otras =
        tarifaRepository.findByTipoHabitacionIdOrderByFechaInicioDesc(tipoHabitacionId).stream()
            .filter(t -> !t.getId().equals(id))
            .collect(Collectors.toList());

    if (otras.isEmpty()) {
      throw new IllegalArgumentException(
          "No se puede eliminar la unica tarifa de un tipo de habitacion");
    }
  ...
  private void publicarPrecioVigente(Integer tipoHabitacionId) {
    ...
    HabitacionEvent msgEvent =
        HabitacionEvent.builder().tipoEvento(TipoEvento.ACTUALIZAR_PRECIO).tarifa(dto).build();
