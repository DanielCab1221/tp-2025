...
public class HabitacionService {
  ...
  /**
   * El precio no es una columna de Habitacion (se resuelve via la tarifa vigente del tipo), asi que
   * el filtro por precio se aplica en memoria despues de traer las que matchean el resto de los
   * criterios por query.
   */
  public List<Habitacion> buscar(
      ...
      Double precioMin,
      Double precioMax) {
    List<Habitacion> candidatas =
        habitacionRepository.buscar(tipoHabitacionId, capacidadMinima, disponible, hotelId);
    ...
    return candidatas.stream()
        .filter(
            h -> {
              ...
              return (precioMin == null || precio >= precioMin)
  ...
  public void enviarHabitacionJms(Habitacion habitacion, boolean isNew) {
    ...
    TipoHabitacion tipoHabitacion =
        tipoHabitacionRepository
            .findById(habitacion.getTipoHabitacion().getId())
    ...
    HabitacionDTO dto =
        HabitacionDTO.builder()
            ...
            // pertenece, para que reservas-svc pueda filtrar la busqueda de
            // disponibilidad
